...
static void coroutine_fn blk_co_pdiscard_entry(void *opaque)
{
    ...
    s->ret = blk_co_pdiscard(s->blk, s->offset, s->bytes, s->flags);
...
int blk_pdiscard(BlockBackend *blk, int64_t offset, int64_t bytes, BdrvRequestFlags flags)
{
    if (qemu_in_coroutine()) {
        
        return blk_co_pdiscard(blk, offset, bytes, flags);
    } else {
        BlkPdiscard s = {
            .poll_state.ctx = qemu_get_current_aio_context(),
            .poll_state.in_progress = true,
